		...
		<%
			for(Order order : orders) {
				...
				<div>
					...
					<p>Email utente: <%= order.getUser().getEmail() %></p>
					<p>Data ordine <%= order.getDate() %></p>
					<p>Totale: <%= order.getAmount() %>€</p>
					<a href ="<%= request.getContextPath() %>/browse/GetOrdersServlet?userid=<%=order.getUser().getId()%>&orderid=<%=order.getId()%>">Dettagli</a>
				</div>
				<hr>
				<%
			}
